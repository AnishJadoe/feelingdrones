...
#define SERVO_MAX_COMD 28672 //It tells us the step we use to control the servo angle
...
#define ONE_ROTATION_ANGLE 4096 // From FDDebug
...
volatile int Target_speed_servo_1, Target_speed_servo_2, Target_speed_servo_3, Target_speed_servo_4;
volatile int Target_speed_servo_5, Target_speed_servo_6, Target_speed_servo_7, Target_speed_servo_8;
...
volatile int servo_1_working_mode, servo_2_working_mode, servo_3_working_mode, servo_4_working_mode;
...
void serial_act_t4_parse_msg_in(void){
  ...
  servo_4_working_mode = myserial_act_t4_in.servo_4_working_mode_int;

  Target_speed_servo_1 = (int) (constrain(myserial_act_t4_in.servo_1_speed_int, -5000, 5000));
  Target_speed_servo_2 = (int) (constrain(myserial_act_t4_in.servo_2_speed_int, -5000, 5000));
  Target_speed_servo_3 = (int) (constrain(myserial_act_t4_in.servo_3_speed_int, -5000, 5000));
  Target_speed_servo_4 = (int) (constrain(myserial_act_t4_in.servo_4_speed_int, -5000, 5000));
  ...
  Target_speed_servo_1 = (int) 0;
  Target_speed_servo_2 = (int) 0;
  Target_speed_servo_3 = (int) 0;
  Target_speed_servo_4 = (int) 0;
  }
...
void writeReadServos(void){
  ...
  byte u8_speed_payload[3] = { 0 }; // Send speed frame
  ...
  if(ack_write_read == 0 && servo_write_read_lock == 0){
    ...
    if(iter_counter_SERVO == 7){ //Send position target to servo 4 or wait in case of servo KILL      
      if(myserial_act_t4_in.servo_arm_int){
        ...
        SplitByte(&u8_Data_1[4],&u8_Data_1[3],0);
        SplitByte(&u8_Data_1[6],&u8_Data_1[5],0);
        SendInstructionServo4(Servo_4_ID, INST_WRITE, u8_Data_1, sizeof(u8_Data_1)); //Servo 2 
        }

        if (servo_4_working_mode == SPEED_CONTROL_MODE){
        u8_speed_payload[0] = SBS_GOAL_SPEED_L;
        SplitByte(&u8_speed_payload[1],&u8_speed_payload[2],Target_speed_servo_4);
        SendInstructionServo4(Servo_4_ID, INST_WRITE, u8_speed_payload, sizeof(u8_speed_payload)); //Servo 2 
        }
...
void WriteSpeedServo(int SERVO_ID){
  byte u8_Data_1[3] = { 0 };
  int Target_speed_servo = 0;
  u8_Data_1[0] = SBS_GOAL_SPEED_L;
  SplitByte(&u8_Data_1[1],&u8_Data_1[2],Target_speed_servo);
  SendInstructionServo4(SERVO_ID, INST_WRITE, u8_Data_1, sizeof(u8_Data_1)); //Servo 4 
}

void WritePositionServo(int SERVO_ID){
  byte u8_Data_1[7] = { 0 };
  int Target_position = 8000;
  u8_Data_1[0] = SBS_GOAL_POSITION_L;
  SplitByte(&u8_Data_1[1],&u8_Data_1[2],Target_position);
  SplitByte(&u8_Data_1[4],&u8_Data_1[3],0);
  SplitByte(&u8_Data_1[6],&u8_Data_1[5],0);
  SendInstructionServo4(SERVO_ID, INST_WRITE, u8_Data_1, sizeof(u8_Data_1)); //Servo 4 
}

void ReadWriteCurrentWorkingMode(int SERVO_ID, int desired_mode){

  int working_mode = 0;
  byte u8_Data_Send[2] = {0};
  u8_Data_Send[0] = SBS_CURRENT_WORKING_MODE;
  u8_Data_Send[1] = 1; // Only 1 byte of data is needed ( I think)

  SendInstructionServo4(SERVO_ID,INST_READ,u8_Data_Send, sizeof(u8_Data_Send));
  delayMicroseconds(TIME_OF_SERVO_TX);
  ...
  uint8_t bitsum_servo = 0;
  for(int i = 2; i < 6; i++){ //Sum the received bits
    bitsum_servo += buffer_servo[i];
  }

  myserial_act_t4_out.servo_4_update_time_us = (int16_t) pos4_time_uS_counter;
  if(255 - bitsum_servo == buffer_servo[6] && buffer_servo[2] == SERVO_ID){
    working_mode = buffer_servo[5];
    pos4_time_uS_counter = 0;
  }
  else{
  ...
  if (working_mode != desired_mode) {
    byte u8_control_mode_frame[2] = { 0 };
    u8_control_mode_frame[0] = SBS_CURRENT_WORKING_MODE;
    u8_control_mode_frame[1] = desired_mode;
    SendInstructionServo4(SERVO_ID,INST_WRITE,u8_control_mode_frame,sizeof(u8_control_mode_frame));
  }
...
void DebugMessageOut(void){
  Serial.println("------------------START-------------------");
  Serial.println((String) myserial_act_t4_out.motor_1_rpm_int);
  Serial.println((String) myserial_act_t4_out.motor_2_rpm_int);
  Serial.println((String) myserial_act_t4_out.motor_3_rpm_int);
  Serial.println((String) myserial_act_t4_out.motor_4_rpm_int);
  Serial.println((String) myserial_act_t4_out.motor_1_error_code_int);
  Serial.println((String) myserial_act_t4_out.motor_2_error_code_int);
  Serial.println((String) myserial_act_t4_out.motor_3_error_code_int);
  Serial.println((String) myserial_act_t4_out.motor_4_error_code_int);
  Serial.println((String) myserial_act_t4_out.motor_1_current_int);
  Serial.println((String) myserial_act_t4_out.motor_2_current_int);
  Serial.println((String) myserial_act_t4_out.motor_3_current_int);
  Serial.println((String) myserial_act_t4_out.motor_4_current_int);
  Serial.println((String) myserial_act_t4_out.motor_1_voltage_int);
  Serial.println((String) myserial_act_t4_out.motor_2_voltage_int);
  Serial.println((String) myserial_act_t4_out.motor_3_voltage_int);
  Serial.println((String) myserial_act_t4_out.motor_4_voltage_int);
  Serial.println((String) myserial_act_t4_out.servo_1_angle_int);
  Serial.println((String) myserial_act_t4_out.servo_2_angle_int);
  Serial.println((String) myserial_act_t4_out.servo_3_angle_int);
  Serial.println((String) myserial_act_t4_out.servo_4_angle_int);
  Serial.println((String) myserial_act_t4_out.servo_5_angle_int);
  Serial.println((String) myserial_act_t4_out.servo_6_angle_int);
  Serial.println((String) myserial_act_t4_out.servo_7_angle_int);
  Serial.println((String) myserial_act_t4_out.servo_8_angle_int);
  Serial.println((String) myserial_act_t4_out.servo_9_angle_int);
  Serial.println((String) myserial_act_t4_out.servo_10_angle_int);
  Serial.println((String) myserial_act_t4_out.servo_1_update_time_us);
  Serial.println((String) myserial_act_t4_out.servo_2_update_time_us);
  Serial.println((String) myserial_act_t4_out.servo_3_update_time_us);
  Serial.println((String) myserial_act_t4_out.servo_4_update_time_us);
  Serial.println((String) myserial_act_t4_out.servo_5_update_time_us);
  Serial.println((String) myserial_act_t4_out.servo_6_update_time_us);
  Serial.println((String) myserial_act_t4_out.servo_7_update_time_us);
  Serial.println((String) myserial_act_t4_out.servo_8_update_time_us);
  Serial.println((String) myserial_act_t4_out.servo_9_update_time_us);
  Serial.println((String) myserial_act_t4_out.servo_10_update_time_us);
  Serial.println((String) myserial_act_t4_out.rolling_msg_out);
  Serial.println((String) myserial_act_t4_out.rolling_msg_out_id);
  Serial.println((String) myserial_act_t4_out.checksum_out);
  Serial.println("--------------------END---------------------");
}
