...
def plot_reference_trajectory(ax):
    ...
    for i, axes_label in enumerate(axes):
        ...
        mocap_line = ax[i].plot(df_mocap.index,df_mocap[axes_label], label='Reference position', linewidth=3)
...
def plot_state_regions(ax, df_command, transition_indices, min_y, max_y):
    ...
    for i in range(4):
        ax[i].imshow([df_command['state'].values], cmap=custom_cmap, aspect='auto',
                      extent=[df_command.index.min(), df_command.index.max(), -10, 10], alpha=0.3)
    ...
    for i in range(4):
        ...
        ax[i].grid()
        ax[i].set_ylim(min_y[i], max_y[i])
        ax[i].set_xlabel('Time [s]')

    return custom_cmap, legend_handles

def plot_sensors(ax, df_sensors, sensor_name_mapping):
    ...
    ax[3].set_yticks([i for i in range(9)])
    ax[3].set_yticklabels([sensor_name_mapping[i] for i in range(9)])
    ax[3].set_xlabel('Time [s]')
    ax[3].invert_yaxis()

def plot_drone_images(ax_top, df_command, transition_indices, transform_ax0_to_ax_top):
    ...
    for i in range(len(transition_indices) - 1):
        if transition_indices[i + 1] - transition_indices[i] < 2:
            continue
        drone_state = int(df_command.iloc[0]['state'])
        x = (transition_indices[i + 1] + transition_indices[i]) / 2
        if drone_state == GRASP or drone_state == SEARCHING:
            ab = AnnotationBbox(getImage(open_drone_path, zoom=0.12),
                                (transform_ax0_to_ax_top.transform((x, 0.01))), boxcoords="axes fraction", frameon=False)
            ax_top.add_artist(ab)
        if drone_state == EVALUATE:
            ab = AnnotationBbox(getImage(closed_drone_path, zoom=0.12),
                                (transform_ax0_to_ax_top.transform((x, 0.01))), boxcoords="axes fraction", frameon=False)
            ax_top.add_artist(ab)
...
def plot_fingers_and_pads(ax2, transform_ax3_to_ax2, df_sensors):
    ...
    for i in range(3):
        ...
        for j in range(3):
            ...
            sensor_id += 1
        finger_id += 3
def draw_drones(transition_indices):
    closed_drone_path = '/home/anish/dev/working/feelingdrones/px4_logger/icons/closed_drone.png'
    open_drone_path = '/home/anish/dev/working/feelingdrones/px4_logger/icons/open_drone.png'
...
def draw_frame(frame):
    ...
    # Get the existing fill_between artists and remove them
    ...
    artist_objects = flatten_list(lines)
    ...
    if frame > 10:
        ...
        transitions = df_command_frame['state'].diff()
        ...
        drone_patches = draw_drones(transition_indices)
...
if __name__ == "__main__":
    ...
    for i,axes in enumerate(axes):
        ax[i].hlines(df_bar[axes].iloc[-1],xmin=min(df_bar.index),xmax=max(df_bar.index),linestyles='dashed', label='Bar position', linewidth=3)
    ...
    animation = FuncAnimation(fig, draw_frame, frames=NUM_FRAMES, interval=INTERVAL,blit=True)
